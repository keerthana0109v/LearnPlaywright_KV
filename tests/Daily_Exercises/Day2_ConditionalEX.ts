...
let passMark:number= 45
function result(grade:number):string{
    if(grade>passMark){
...
// based on its sides.
function findTriangleType(s1:number,s2:number,s3:number):string{
...
function checkPalindrome(originalStr:string):string{
    let reversedStr=""
